...
describe("Redirect Handling Basic Tests", () => {
	...
	test("should follow multiple redirects", async () => {
		const response = await testFetch("/redirect/3");
	...
	test("should handle redirect with query params", async () => {
		const response = await testFetch("/redirect-to?url=/get?test=123");
	...
	test("should handle redirect limit", async () => {
		...
		const response = await testFetch("/redirect/5");
	...
	test("should handle absolute URL redirects", async () => {
		const response = await testFetch("/absolute-redirect/1");
	...
	test("should handle relative URL redirects", async () => {
		const response = await testFetch("/relative-redirect/1");
...
describe("Redirect Security Tests", () => {
	...
	test.only("should stop at max redirects (20)", async () => {
		const response = await testFetch("https://httpbin.org/redirect/21");

		// Should stop at redirect limit and return last redirect response
