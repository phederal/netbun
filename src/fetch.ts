...
export function parseProxyUrl(proxyUrl: string) {
	try {
		...
		// Validate Protocol - support socks5, socks5h (alias), http, https.
		// socks5h is treated identically to socks5: we already default to remote
		// DNS resolution (ATYP=0x03) unless `resolveDnsLocally` is set.
		const protocol = parsed.protocol;
		const validProtocols = ["socks5:", "socks5h:", "http:", "https:"];
		...
		// Normalize socks5h to socks5 — they share the same handshake; the only
		// difference is who resolves DNS, and that is governed by `resolveDnsLocally`.
		const normalizedProtocol =
			protocol === "socks5h:" ? "socks5" : protocol.slice(0, -1);
		return { host, port, user, password, protocol: normalizedProtocol };
	} catch (err) {
		throw new Error(
